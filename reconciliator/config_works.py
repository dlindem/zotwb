...
mediawiki_api_endpoint = 'https://monumenta.wikibase.cloud/w/api.php'

# SPARQL endpoint
wikibase_sparql_endpoint = 'https://monumenta.wikibase.cloud/query/sparql'

# Name of the Wikibase instance
wikibase_name = 'MLV'

# URL of the main page of the Wikibase instance
wikibase_main_page = 'https://monumenta.wikibase.cloud/wiki/Main_Page'
...
user_agent = 'OpenRefine-MLV reconciliation interface'

# Regexes and group ids to extracts Qids and Pids from URLs
import re
q_re = re.compile(r'(<?https?://monumenta.wikibase.cloud/(entity|wiki)/)?(Q[0-9]+)>?')
q_re_group_id = 3
p_re = re.compile(r'(<?https?://monumenta.wikibase.cloud/(entity/|wiki/Property:))?(P[0-9]+)>?')
p_re_group_id = 3
...
identifier_space = 'https://monumenta.wikibase.cloud/entity/'
schema_space = 'https://monumenta.wikibase.cloud/prop/direct/'
...
qid_url_pattern = 'https://monumenta.wikibase.cloud/wiki/{{id}}'
...
service_name = 'MLV Reconciliation'
...
headers = {
    'User-Agent':service_name + ' (OpenRefine-MLV reconciliation service)',
}
...
SELECT ?child WHERE { ?child xdp:P4* xwb:$qid }
"""
